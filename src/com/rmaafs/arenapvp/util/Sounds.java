...
public enum Sounds {
    ...
    /**
     * Arreglo con el nombre del sound en varias versiones.
     */
    private String[] soundNames;
    /**
     * Hash donde se guardaará sonidos cargados anteriormente.
     */
    public static HashMap<String, Sound> cached = new HashMap<>();

    Sounds(String... versionDependentNames) {
        this.soundNames = versionDependentNames;
    }

    /**
     * Función que reproduce el sonido a un jugador
     * @param player Jugador que escuchará
     * @param song Sonido a reproducir
     */
    public static void song(Player player, String song) {
        //Si el sonido ya ha sido cargado anteriormente...
        if (cached.containsKey(song)) {
            //Lo reproducimos
            player.playSound(player.getLocation(), cached.get(song), 1.0F, 1.0F);
        }

        //Ciclamos todos los sonidos disponibles en el enum
        for (Sounds s : Sounds.values()) {
            //Ciclamos el nombre de ese sonido en las versiones disponibles
            for (String vName : s.soundNames) {
                //El sonido que buscamos, es el mismo al que estamos ciclando?
                if (song.toUpperCase().equals(vName)) {
                    //Ciclamos todos los nombres que tiene en cada versión este sonido
                    for (String vName2 : s.soundNames) {
                        try {
                            //Intentamos reproducir ese sonido
                            player.playSound(player.getLocation(), org.bukkit.Sound.valueOf(vName2), 1.0F, 1.0F);
                            //Si el sonido fue correcto, lo guardamos en caché para ahorrar procesamiento.
                            if (!cached.containsKey(song)) {
                                cached.put(song, org.bukkit.Sound.valueOf(vName2));
                            }
                        } catch (Exception e) {
                            /**
                             * En caso de que salga un error de Spigot que el sonido no se encuentra,
                             * ignoramos el error y en el siguiente ciclo, intentaremos obtener
                             * el sonido pero en el nombre que tiene en otra versión.
                             */
                        }
                    }
